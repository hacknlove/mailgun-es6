...
class FormData {
  ...
  addData(name, value, type, buffer) {
    //This function adds data to the dataStream.
    //If you need this function to read the file for you.. Yes. It's blocking.
    //Yes, that's how I designed it. Is there a reason to turn this into a
    // complicated promise or callback situation when you can read the file
    // yourself and pass the buffer as a parameter?
    ...
    if (buffer !== undefined) {
      // buffer passes
      this.dataStream.push(`Content-Disposition: form-data; name="${name}"; filename="${value}"`);
